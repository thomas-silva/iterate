---
name: propose-parent
description: Propose a Linear parent for review after review-parent saves a ready-to-propose verdict. Create or update a PR with a concise outcome and implementation debrief plus curated media evidence, then move the parent from In Progress to In Review. Use when the user runs /propose-parent or asks to prepare a completed parent implementation for PR review.
---

# Propose Parent

A ready-to-propose verdict from `review-parent` means the parent can be proposed. Create the PR and move the Linear parent to In Review. Do not merge or mark the parent Done.
...
1. Require a saved ready-to-propose verdict from [review-parent](../review-parent/SKILL.md) covering the current parent-branch head and intended PR base commits. If it is absent, blocked, or either commit changed, stop and tell the user to run `/review-parent`. Do not publish the PR or advance the parent.

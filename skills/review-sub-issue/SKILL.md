...
description: Review one implemented Linear child issue for alignment, architecture, worthwhile simplification, and live-system acceptance. Surface ranked findings before edits, apply safe child-local simplifications, then comment what remains on the issue. Use when the user runs /review-sub-issue, asks to review a Linear sub-issue or child issue, or to review-step a Linear issue.
---
...
4. Output ranked findings to the user before any edits. If none, return `No remaining findings.` and stop.
...
8. Set the child to Done and `save_comment` remaining findings. Reply with the child URL and branch. Do not edit the parent, or create a sibling.
9. Recap to the user.
